...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        # Injectable for tests; the defaults need Docling and an OpenAI key.
        self.extractor = extractor or create_pdf_extractor(self.extractor_config)
        self.chunker = chunker or create_chunker(self.chunker_config)
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        embedded_chunks = await self.aembed_chunks(chunks=main_chunks, model=get_embedding_model())
    ...
    async def aembed_chunks(
        self, chunks: list[DocumentChunk], model: str = "text-embedding-3-small"
    ) -> list[DocumentChunk]:
    ...
    async def _save_to_postgres(
        ...
        async with db_pool.acquire() as conn:  # noqa: SIM117
...
async def main():
    ...
    parser.add_argument(
        "--chunk-size", type=int, default=850, help="Chunk size for splitting documents"
    )
    ...
    parser.add_argument(
        "--sql-schema-path", "-sql", default="sql/schema.sql", help="Path to SQL schema file"
    )
    ...
    try:
        ...
        for result in results:
            if result.chunks_created > 0:
                logger.info(
                    f"{result.title}: {result.chunks_created} chunks ({result.processing_time_ms / 1000:.1f}s)"
                )
            else:
