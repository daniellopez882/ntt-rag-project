...
class PDFSemanticChunker:
    ...
    def __init__(self, config: ChunkingConfig):
        ...
        if config.use_semantic_splitting:
            # Built only here: constructing it unconditionally required an OpenAI key
            # even for recursive chunking.
        ...
        for i, chunk in enumerate(chunks):
            ...
            if len(text) >= self.config.min_chunk_size:
                chunk.metadata.update(
                    {"chunk_index": i, "total_chunks": len(chunks), "chunk_size": len(text)}
                )
                final_chunks.append(
                    DocumentChunk(
                        content=text,
    ...
    def chunk_pdf_documents(self, documents: list[Document]) -> list[Document]:
        ...
        for doc in documents:
            ...
            chunks = self.chunk_content(
                content=doc.page_content, title=title, source=source, metadata=doc.metadata
            )
