...
| 17 | Seven dependencies with published advisories: `starlette`, `idna`, `python-dotenv`, `click`, `requests`, `urllib3`, and `pydantic-ai-slim` pinned `<1` | CI's `pip-audit` found them the first time it ran on GitHub. All seven are on fixed releases now and `pip-audit` reports nothing; the pydantic-ai move from 0.7.6 to 1.x was checked against a real `Agent` with a live model object, not only against the suite's stub |
